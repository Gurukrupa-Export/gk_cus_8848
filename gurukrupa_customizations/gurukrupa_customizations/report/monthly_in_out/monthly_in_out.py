...
def get_data(filters=None):
	...
	# 		time(at.in_time) as in_time, time(at.out_time) as out_time, 
...
def get_totals(data, employee):	
	...
	for row in data:
		...
		if "net_wrk_hrs" not in totals:
			totals["net_wrk_hrs"] = timedelta(0)
		net_wrk_hrs_str = row.get("net_wrk_hrs") or "00:00"
		net_wrk_hrs_timedelta = str_to_timedelta(net_wrk_hrs_str)
		totals["net_wrk_hrs"] += net_wrk_hrs_timedelta

		if "total_pay_hrs" not in totals:
			totals["total_pay_hrs"] = timedelta(0)
		total_pay_hrs_str = row.get("total_pay_hrs") or "00:00"
		total_pay_hrs_timedelta = str_to_timedelta(total_pay_hrs_str)
		totals["total_pay_hrs"] += total_pay_hrs_timedelta
		# totals["total_pay_hrs"] += (row.get("total_pay_hrs") or timedelta(0))


		if "ot_hours" not in totals:
			totals["ot_hours"] = timedelta(0)
		ot_hours_str = row.get("ot_hours") or "00:00"
		ot_hours_timedelta = str_to_timedelta(ot_hours_str)
		totals["ot_hours"] += ot_hours_timedelta
		...
		if "spent_hours" not in totals:
			totals["spent_hours"] = timedelta(0)
		spent_hours_str = row.get("spent_hours") or "00:00"
		spent_hours_timedelta = str_to_timedelta(spent_hours_str)
		totals["spent_hours"] += spent_hours_timedelta
...
def process_data(data, filters):
	...
	for row in data:
		...
		shift_hours_in_min = row.shift_hours*60
		hours, minutes = map(int, row.net_wrk_hrs.split(':'))
		time_float = hours + minutes / 60

		net_wrk_hrs_timedelta = timedelta(hours=hours, minutes=minutes)

		if time_float*60 > shift_hours_in_min or (shift_hours_in_min - (time_float*60)) < 0.6:
			row.net_wrk_hrs = timedelta(hours=row.shift_hours)

		ot_hours_str = row.get('ot_hours')
		if ot_hours_str:
			ot_hours_timedelta = str_to_timedelta(ot_hours_str)
		else:
			ot_hours_timedelta = timedelta(0)


		# row["total_pay_hrs"] = net_wrk_hrs_timedelta  + (row.get("ot_hours") or timedelta(0))
		row["total_pay_hrs"] = net_wrk_hrs_timedelta + ot_hours_timedelta
	...
	for date in date_range:
		...
		try:
			time_obj = datetime.strptime(str(row.total_pay_hrs), "%H:%M:%S")
			time_hhmm = time_obj.strftime("%H:%M")
			row['total_pay_hrs'] = time_hhmm
		except:
			row['total_pay_hrs'] = "00:00"
